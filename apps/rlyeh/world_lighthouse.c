...
static void lighthouse_build(scene **out_s, scene_camera **out_cam,
    ...
    /* ===== Sea — infinite reflective plane; fog melts it into the horizon. */
    scene_add_plane(s, (scene_plane){
        .point = {0, 0, 0}, .normal = {0, 1, 0}, .material = m_sea,
    });

    /* ===== Islet — two half-buried dark spheres; mostly submerged so they
     * read as a low rocky mound at the tower's foot rather than a wall. */
    scene_add_sphere(s, (scene_sphere){
        .center = { 0.0f, -11.0f, 95.0f }, .radius = 13.0f, .material = m_rock,
    });
    scene_add_sphere(s, (scene_sphere){
        .center = { 8.0f, -10.0f, 101.0f }, .radius = 8.0f, .material = m_rock,
    });

    /* ===== Lighthouse — tower + gallery + lantern + roof. Set well ahead so
     * the whole silhouette frames up when you wake looking down +Z. */
    scene_add_cylinder(s, (scene_cylinder){
        .center = {0, 16.0f, 95.0f}, .axis = {0, 1, 0},
        .radius = 3.4f, .half_height = 16.0f, .material = m_tower,
    });
    scene_add_cylinder(s, (scene_cylinder){
        .center = {0, 32.6f, 95.0f}, .axis = {0, 1, 0},
        .radius = 4.3f, .half_height = 1.1f, .material = m_gallery,
    });
    vector lantern = { 0.0f, 34.4f, 95.0f };
    scene_add_sphere(s, (scene_sphere){
        .center = lantern, .radius = 2.2f, .material = m_lantern,
    });
    scene_add_cone(s, (scene_cone){                 /* roof: apex up, opens down */
        .apex = {0, 38.4f, 95.0f}, .axis = {0, -1, 0},
    ...
    /* ===== Seam stalk — one coral cone that grows taller with wrongness;
     * starts as a sub-unit nub so it's already (barely) there at w=0.15. */
    SEAM_BASE = (vector){ 4.0f, 0.0f, 88.0f };
