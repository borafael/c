...
int main(int argc, char *argv[]) {
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type == SDL_KEYDOWN) {
                ...
                if (k == SDLK_LEFTBRACKET) {
                    int n = postfx_palette_count();
                    palette_idx = (palette_idx + n - 1) % n;
                    fprintf(stderr, "Palette: %s\n", postfx_palette_at(palette_idx)->name);
                }
                if (k == SDLK_RIGHTBRACKET) {
                    palette_idx = (palette_idx + 1) % postfx_palette_count();
                    fprintf(stderr, "Palette: %s\n", postfx_palette_at(palette_idx)->name);
                }
                if (k == SDLK_h) {
                    dither_on = !dither_on;
        ...
        if (posterize_on) postfx_posterize(pixels, render_w, render_h);
        if (palette_on) postfx_quantize(pixels, render_w, render_h,
                                        postfx_palette_at(palette_idx), dither_on);
        ...
        if (now - fps_last >= 1000) {
            ...
            const char *pal_label = palette_on ? postfx_palette_at(palette_idx)->name : "off";
