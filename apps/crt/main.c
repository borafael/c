/* CRT / VHS / film stack demo.
 *
 * Stacks four small post-processes on the raytraced output:
 *   1. postfx_chromatic   — horizontal R/B channel separation
 *   2. postfx_scanlines   — alternating-row brightness dim
 *   3. postfx_vignette    — radial corner darkening
 *   4. postfx_grain       — animated film grain
 *
 * Each pass is independent; toggle individually to A/B which one is
 * doing what. Order matters: chromatic before scanlines (so the
 * shifted channels also get scanline-dimmed), vignette before grain
 * (so the grain stays visible in the dimmed corners).
 *
 * Controls:
 *   ESC          quit
 ...
 *   V            toggle vignette
 *   G            toggle grain
 *   - / =        less / more chromatic shift
 *   [ / ]        thinner / thicker scanline period
 *   N            freeze grain (static vs animated)
...
int main(int argc, char *argv[]) {
    ...
    int fullscreen = 0;
    SDL_Window *window = SDL_CreateWindow("CRT Stack",
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type == SDL_KEYDOWN) {
                ...
                if (k == SDLK_s) {
                    scan_cfg.enabled = !scan_cfg.enabled;
                    fprintf(stderr, "Scanlines: %s\n", scan_cfg.enabled ? "on" : "off");
                }
                if (k == SDLK_c) {
                    chrom_cfg.enabled = !chrom_cfg.enabled;
                    fprintf(stderr, "Chromatic: %s\n", chrom_cfg.enabled ? "on" : "off");
                }
                if (k == SDLK_v) {
                    vig_cfg.enabled = !vig_cfg.enabled;
                    fprintf(stderr, "Vignette: %s\n", vig_cfg.enabled ? "on" : "off");
                }
                if (k == SDLK_g) {
                    grain_cfg.enabled = !grain_cfg.enabled;
                    fprintf(stderr, "Grain: %s\n", grain_cfg.enabled ? "on" : "off");
                }
                if (k == SDLK_n) {
                    grain_animated = !grain_animated;
                    fprintf(stderr, "Grain: %s\n", grain_animated ? "animated" : "static");
                }
                if (k == SDLK_MINUS || k == SDLK_KP_MINUS) {
                    if (chrom_cfg.shift_pixels > -8) chrom_cfg.shift_pixels--;
                    fprintf(stderr, "Chromatic shift: %d\n", chrom_cfg.shift_pixels);
                }
                ...
                if (k == SDLK_LEFTBRACKET) {
                    if (scan_cfg.period > 1) scan_cfg.period--;
                    fprintf(stderr, "Scanline period: %d\n", scan_cfg.period);
                }
                if (k == SDLK_RIGHTBRACKET) {
                    if (scan_cfg.period < 16) scan_cfg.period++;
                    fprintf(stderr, "Scanline period: %d\n", scan_cfg.period);
                }
        ...
        /* Order: chromatic → scanlines → vignette → grain. The earlier
         * passes' output feeds the later ones, which is exactly what
         * we want — grain settles on top of everything. */
        ...
        grain_cfg.seed = grain_animated ? frame_now : 0xC0FFEE;
        ...
        if (now - fps_last >= 1000) {
            ...
            snprintf(title_buf, sizeof(title_buf),
                     "CRT Stack - %s %s S=%d C=%d V=%d G=%d shift=%d period=%d %d FPS (rt=%.1fms fx=%.1fms)",
                     rt_renderer_name(active), PRESETS[preset].name,
                     scan_cfg.enabled, chrom_cfg.enabled,
                     vig_cfg.enabled, grain_cfg.enabled,
                     chrom_cfg.shift_pixels, scan_cfg.period,
                     fps_frames, avg_render, avg_fx);
            SDL_SetWindowTitle(window, title_buf);
            fprintf(stderr, "[%s %s S=%d C=%d V=%d G=%d shift=%d period=%d] %d FPS, rt=%.1fms fx=%.1fms\n",
                    rt_renderer_name(active), PRESETS[preset].name,
                    scan_cfg.enabled, chrom_cfg.enabled,
                    vig_cfg.enabled, grain_cfg.enabled,
                    chrom_cfg.shift_pixels, scan_cfg.period,
