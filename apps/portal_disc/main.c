/* portal_disc: object-traversal demo, single disc portal pair.
 *
 * The minimal showcase of the portal-traversal feature — one sphere
 * tagged with one paired-rigid disc portal, oscillating back and forth
 * through the entry. Front-of-A half stays at the original position;
 * back-of-A half emerges in front of B.
 *
 * For the multi-portal, mesh, and sphere-portal variants see the
 * sibling apps/portal_multi, apps/portal_mesh, apps/portal_sphere.
 * apps/portals integrates all four into one scene. */
...
#define TRAVELER_MIN_X  -3.6f
#define TRAVELER_MAX_X  -0.4f
#define TRAVELER_PERIOD  7.0f
...
static void build_scene(scene **scene_out, scene_camera **camera_out) {
    ...
    /* A pair of background reference balls give the eye a depth anchor. */
...
int main(int argc, char *argv[]) {
    ...
    SDL_Window *win = SDL_CreateWindow("portal_disc — sphere through one disc portal",
    ...
    while (running) {
        ...
        if (traveler_idx >= 0) {
            float u = (sinf(t * 2.0f * (float)M_PI / TRAVELER_PERIOD) + 1.0f) * 0.5f;
            sc->spheres[traveler_idx].center.x =
