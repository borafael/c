/* portal_multi: object-traversal demo, multi-portal + de-dup.
 *
 * Two paired-rigid disc portals (A↔B at +X normal, C↔D at +Z normal)
 * with A and C co-located at the same world corner. A single sphere
 * tagged with both portals traces a circle in the X-Z plane around the
 * A/C corner — so over one lap it visits every combination of "front
 * of A / behind A" × "front of C / behind C":
 *
 *   Q1 front of both : full original.
 *   Q2 behind A only : full virtual at B.
 *   Q3 behind both   : full virtual at B (the higher-priority slot
 *                       owns the overlap region; D shows nothing).
 *   Q4 behind C only : full virtual at D.
 *
 * Quadrant crossings are the half-emerge moments.
 *
 * For the single-portal version see apps/portal_disc. */
...
#define TRAV_Y         0.5f
#define TRAV_RADIUS    0.5f
#define TRAV_PATH_R    1.5f
#define TRAV_CX       -2.0f
#define TRAV_CZ       -2.0f
#define TRAV_PERIOD   10.0f
...
static void build_scene(scene **scene_out, scene_camera **camera_out) {
    ...
    int disc_A = 0, disc_B = 1, disc_C = 2, disc_D = 3;
    ...
    int pC = scene_add_portal(sc, (scene_portal){
        .kind = SCENE_PORTAL_PAIRED_RIGID,
        .partner_kind = SCENE_PRIM_DISC, .partner_index = disc_D});
    int pD = scene_add_portal(sc, (scene_portal){
        .kind = SCENE_PORTAL_PAIRED_RIGID,
        .partner_kind = SCENE_PRIM_DISC, .partner_index = disc_C});
    ...
    int mC = scene_add_material(sc, (scene_material){
        .albedo = {230, 200,  80}, .portal_index = pC});
    int mD = scene_add_material(sc, (scene_material){
        .albedo = { 80, 230, 130}, .portal_index = pD});
    ...
    scene_add_disc(sc, (scene_disc){
        .center = {-2.0f, 0.5f, -2.0f}, .normal = {0, 0, 1},  /* co-located with A */
        .radius = 1.0f, .material = mC});
    scene_add_disc(sc, (scene_disc){
        .center = { 2.0f, 0.5f, -2.0f}, .normal = {0, 0, 1},
        .radius = 1.0f, .material = mD});

    int m_trav = scene_add_material(sc, (scene_material){
        .albedo = {255, 215, 60}, .portal_index = -1});
    traveler_idx = scene_add_sphere(sc, (scene_sphere){
        .center = {TRAV_CX + TRAV_PATH_R, TRAV_Y, TRAV_CZ},
        .radius = TRAV_RADIUS,
        .material = m_trav,
        .portal_disc1 = {disc_A + 1, disc_C + 1},
    });
...
int main(int argc, char *argv[]) {
    ...
    SDL_Window *win = SDL_CreateWindow("portal_multi — sphere across two portals",
    ...
    while (running) {
        ...
        if (traveler_idx >= 0) {
            float theta = t * (2.0f * (float)M_PI / TRAV_PERIOD);
            sc->spheres[traveler_idx].center.x = TRAV_CX + TRAV_PATH_R * cosf(theta);
            sc->spheres[traveler_idx].center.z = TRAV_CZ + TRAV_PATH_R * sinf(theta);
        }
