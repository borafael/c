/* Bloom raytrace demo.
 *
 * Renders a scene with a few deliberately bright unlit "neon" spheres,
 * then runs postfx_bloom to extract pixels above a luminance threshold,
 * blur them at half resolution, and add the result back additively. The
 * mirror sphere picks up the neon reflections so the bloom kicks in
 * through bounces too — a free showcase of the renderer's reflection
 * pipeline feeding the post-process.
 *
 * Controls:
 *   ESC          quit
 ...
 *   B            toggle bloom on/off (compare raw vs bloomed)
 *   - / =        reduce / increase bloom intensity
 *   [ / ]        reduce / increase blur radius
 *   ; / '        lower / raise the brightness threshold
 *   T            cycle blur iterations (1..4) — wider, softer falloff
...
static void build_scene(scene **scn, scene_camera **cam) {
    *scn = scene_create();
    ...
    /* Three "neon" materials: unlit + saturated colour past the bloom
     * threshold so they glow regardless of lighting. The lit greys and
     * the floor stay below threshold, so only the neons (and their
     * mirror reflections) bloom. */
    int m_neon_pink = scene_add_material(*scn, (scene_material){
        .albedo = {255, 80, 200}, .unlit = 1 });
    int m_neon_cyan = scene_add_material(*scn, (scene_material){
        .albedo = {80, 240, 255}, .unlit = 1 });
    int m_neon_yellow = scene_add_material(*scn, (scene_material){
    ...
    int m_concrete = scene_add_material(*scn, (scene_material){
        .albedo = { 90,  90, 100},
    });

    scene_add_sphere(*scn, (scene_sphere){
        .center = {-2.4f, 0.6f, -0.5f}, .radius = 0.6f, .material = m_neon_pink });
    scene_add_sphere(*scn, (scene_sphere){
        .center = {2.0f, 0.8f, -1.0f}, .radius = 0.8f, .material = m_neon_cyan });
    scene_add_sphere(*scn, (scene_sphere){
        .center = {0.0f, 0.5f, 1.5f}, .radius = 0.5f, .material = m_neon_yellow });

    /* Mirror sphere picks up neon reflections; it's the demo for "bloom
     * works through bounces too". */
    scene_add_sphere(*scn, (scene_sphere){
        .center = {0.0f, 2.0f, -1.0f}, .radius = 1.1f, .material = m_mirror });

    scene_add_box(*scn, scene_box_aabb((vector){-3.5f, -0.5f, 1.5f},
                                       (vector){-2.5f,  0.8f, 2.5f}, m_concrete));
    scene_add_cylinder(*scn, (scene_cylinder){
        .center = {2.5f, 0.5f, 2.0f}, .axis = {0.0f, 1.0f, 0.0f},
        .radius = 0.45f, .half_height = 1.0f, .material = m_concrete });
    ...
    /* Dim ambient + gentle key light: keep lit surfaces well under the
     * bloom threshold so the unlit neons are clearly the only bloom
     * sources. */
    scene_set_ambient(*scn, 0.18f);
...
int main(int argc, char *argv[]) {
    ...
    int fullscreen = 0;
    SDL_Window *window = SDL_CreateWindow("Bloom Raytrace",
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type == SDL_KEYDOWN) {
                ...
                if (k == SDLK_SEMICOLON) {
                    bloom_cfg.threshold -= 0.05f;
                    if (bloom_cfg.threshold < 0.05f) bloom_cfg.threshold = 0.05f;
                    fprintf(stderr, "Threshold: %.2f\n", bloom_cfg.threshold);
                }
                if (k == SDLK_QUOTE) {
                    bloom_cfg.threshold += 0.05f;
                    if (bloom_cfg.threshold > 1.0f) bloom_cfg.threshold = 1.0f;
                    fprintf(stderr, "Threshold: %.2f\n", bloom_cfg.threshold);
                }
                if (k == SDLK_t) {
                    bloom_cfg.iterations = (bloom_cfg.iterations % 4) + 1;
                    fprintf(stderr, "Iterations: %d\n", bloom_cfg.iterations);
                }
        ...
        Uint32 b_done = SDL_GetTicks();
        render_ms_accum += r_done - r_start;
        bloom_ms_accum  += b_done - r_done;
        ...
        if (now - fps_last >= 1000) {
            ...
            float avg_bloom  = (fps_frames > 0) ? (float)bloom_ms_accum  / (float)fps_frames : 0.0f;
            snprintf(title_buf, sizeof(title_buf),
                     "Bloom Raytrace - %s %s bloom=%s thr=%.2f int=%.2f r=%d it=%d %d FPS (rt=%.1fms, bloom=%.1fms)",
                     rt_renderer_name(active), PRESETS[preset].name,
                     bloom_cfg.enabled ? "on" : "off",
                     bloom_cfg.threshold, bloom_cfg.intensity,
                     bloom_cfg.radius, bloom_cfg.iterations,
                     fps_frames, avg_render, avg_bloom);
            SDL_SetWindowTitle(window, title_buf);
            fprintf(stderr, "[%s %s bloom=%d thr=%.2f int=%.2f r=%d it=%d] %d FPS, rt=%.1fms, bloom=%.1fms\n",
                    rt_renderer_name(active), PRESETS[preset].name,
                    bloom_cfg.enabled, bloom_cfg.threshold, bloom_cfg.intensity,
                    bloom_cfg.radius, bloom_cfg.iterations,
                    fps_frames, avg_render, avg_bloom);
            fps_frames = 0;
            render_ms_accum = 0;
